...
If the first argument given to this script does not start with a hyphen (-), it will assume you're trying to launch it with the latter functionality
   ...
   POSITIONAL ARGUMENTS:
	...
	ARGUMENTS:
		n|ne|new <container name> <odoo version> <port> 
			creates and builds a new container with the given options

		l|li|lis|list option [--all]
			lists status of containers built by this script.
			lists all docker containers if --all is specified
   ...
   OPTIONS:
	-n, --new <container name> <odoo version> <port> 
		creates and builds a new container with the given options
	...
	-l, --list 
		lists the status of the current built containers

	-s, --start <container name> [<odoo server option 1> <odoo server option 2>...]
		starts [CONTAINER]'s Odoo server with [OPTIONS]
	...
	-e, --enter <container name> 
		logs into <container name> with a bash shell session

	-r, --run <container name> <user> <command> 
		runs <command> inside <container name> 

	-c, --clone <container> <script> <version> 
		launches a predefined clone script. see repos/README.md
...
function start(){
	...
	# shellcheck disable=SC2124
	ARGS="$@"
	docker exec -it "$container" runuser odoo -c "cd; cd custom/etc; ./generate-conf.sh; ./run-odoo.sh $ARGS"
}
function run(){
	container=$1
	shift
...
function kill(){
	echo -e "stopping $1"
	cd "$CONTAINERS"/"$1" || exit; docker-compose stop
	echo -e "$1 stopped"
}
...
function new(){
		...
		mkdir -p "$CONTAINER"/custom/{etc,addons,data}
